...
public class JsonModel {
  ...
  public static List<String> getJsonModels(File resourcePack) {
    try (FileSystem jarFs = FileSystems.newFileSystem(URI.create("jar:" + resourcePack.toURI()),
        Collections.emptyMap())) {
      Path blockModels = jarFs.getPath("/assets/minecraft/models/block");
      try (Stream<Path> paths = Files.walk(blockModels)) {
        return paths.filter(Files::isRegularFile)
            .filter(path -> path.toString().endsWith(".json"))
            .map(path -> {
              String filename = blockModels.relativize(path).toString();
